	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(android.os.Message msg) {
			if(msg.what == REFRESH) {
				...
				Toast.makeText(CityViewActivity.this, "服务器已断开，请稍候重试", Toast.LENGTH_SHORT).show();
			}
		};
	};
...
//	private MyAdapter adapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		type = getIntent().getIntExtra(WHATTYPE, VIEW);
		if(type == VIEW) {
			cityName = getIntent().getStringExtra(CITYNAME);
			vPinYin = getIntent().getStringExtra(VIEWPINYIN);
			vName = getIntent().getStringExtra(VIEWNAME);
			title = vName + " in " + cityName;
			sendMessage = "getMoodsOfViewSortByTime&&" + vPinYin + "&&";
		}
...
//		System.out.println("1111111111111111111111111111111"+date[0]);
		...
		tvAdd.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Intent intent = new Intent(CityViewActivity.this,ZoneActivity.class);
...
//        });
	...
	class MyAdapter extends BaseExpandableListAdapter{
		...
		public View getGroupView(int groupPosition, boolean isExpanded,
			...
			if(convertView == null){
    		if(groupPosition==1){
    			LayoutInflater inflater = (LayoutInflater) CityViewActivity.this
		...
		like.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(numlike%2==1){
					...
					Toast.makeText(CityViewActivity.this, "已加入“我想去”列表", Toast.LENGTH_SHORT).show();
				}else{
					like.setImageResource(R.drawable.like);
					Toast.makeText(CityViewActivity.this, "已取消收藏", Toast.LENGTH_SHORT).show();
				}
			}
		});
