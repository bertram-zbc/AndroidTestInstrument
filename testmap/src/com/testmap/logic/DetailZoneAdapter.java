...
public class DetailZoneAdapter extends BaseAdapter {
		...
		public void handleMessage(android.os.Message msg) {
			if(msg.what == 0x1) {
				...
				if(mood != null) {
					...
					mood.zoneadapter.notifyDataSetChanged();
				}
	...
	public DetailZoneAdapter(Context ctx, ArrayList<Mood> moodsList) {
		this.mContext = ctx;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if(mood.zoneadapter == null) {
			mood.zoneadapter = new MyCommentsAdapter(mContext, mood.cList);
		}
		if(mood.isZan()) {
		...
		holder.comments_listview.setAdapter(mood.zoneadapter);
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			holder.tt_author.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if(author.equals(ClientThread.getUserName())) {
						Intent intent = new Intent(DetailZoneAdapter.this.mContext,MyPersonalActivity.class);
						DetailZoneAdapter.this.mContext.startActivity(intent);
					} else {
						FragmentTransaction ft = ((Activity) mContext).getFragmentManager().beginTransaction();
						DialogFragment newFragment = new PersonFragment();
						Bundle args = new Bundle();
